...
export class AuthProvider {
  ...
  constructor(
    ...
    this.loginProvider$ = afAuth.authState.map(user => {
      if (!user) return {};
      return user.providerData.reduce((a, e) => {a[e.providerId.replace('.', '_')] = true; return a}, {});
    }).do(e => console.log(`loginProvider$ ${JSON.stringify(e)}`), e => console.error(`loginProvider$ ${JSON.stringify(e)}`), )
  }

  public linkToGoogle() {
    return this.googlePlus.login(this.GOOGLE_OPTIONS)
    .then(res => {
      ...
      return user.linkWithCredential(credential)
        .catch((err : firebase.auth.Error) => {
          if (err.code === 'auth/credential-already-in-use' ||
            ...
            console.log('recovering by errorcode == auth/credential-already-in-use')
            return this.afAuth.auth.signInWithCredential(credential);
          }
